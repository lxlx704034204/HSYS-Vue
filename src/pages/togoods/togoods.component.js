...
export default {
  ...
  methods: {
          ...
          if (data.code == 0) {
            if (data.data != "") {
              ...
              for(var i=0;i<that.datatime.length;i++){
                ...
                }else if(that.datatime[i].orderEvent.value == "payment"){
                  that.paymenttime =that.datatime[i].createDate                  
                }else if(that.datatime[i].orderEvent.value == "deliver"){
                  that.deliverytime =that.datatime[i].createDate
                }
              }
    ...
    confirm(){
      axios.post(baseURL + 'order/' + this.orderid + "/receive").then(function (res) {
        if(res.data.code==0){
          this.$route.push({pash:"/companycenter/tosigning",query: {id:'this.orderid'}})
        }
