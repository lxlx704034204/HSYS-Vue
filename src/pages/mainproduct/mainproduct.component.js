export default {
    ...
    created() {
        ...
        //  分类
        that.axios.get("customerproduct/category?id=" + that.userId, { params: { noInterceptor: 1 } })
            .then(function(data) {
    ...
    methods: {
        mainlist() {
            //  用户产品
            var that = this;
            this.axios.get("customerproduct/all?user_id=" + this.userId + '&size=12&current=' + this.pageNo + '&cid=' + this.cid, { params: { noInterceptor: 1 } })
                .then(function(data) {
                    if (data.code == 0) {
