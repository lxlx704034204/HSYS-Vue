export default {
    ...
    created() {
        console.log(this.$store.state.loginName,'name')
        // 调用数据
    ...
    methods: {
        ...
        login_btn() {
            if (this.phone == '' && this.verif == "") {
                ...
                this.axios.post('msg/verifyRandomCodeNoLogin', msgs, { params: { noInterceptor: 1 } }).then((res) => {
                    if (res.code == 500) {
                        that.verif = '';
                        that.verifs = true;
                        that.verifa = "验证码错误";
        ...
        phoneq() {
            ...
            if (this.phone == '') {
                ...
                this.phonea = "";
                var that = this;
                this.axios.post('updatepaw/checkPhone?login=' + this.$store.state.loginName + '&phone=' + that.phone).then((res) => {
                    console.log(res, '2');
                    if (!!res) {
                        that.phones = true;
