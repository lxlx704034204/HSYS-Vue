  ...
  methods: {
    ...
    loginbtn(e) {
      if (this.username == '' && this.pwd == '') {
        ...
        if (!/^\w{5,14}$/.test(this.username)) {
          this.usernametext = "请输入4-15位的正确账号";
          this.nameisshow = true;
          return false;
        }
        this.axios.post('login?login=' + that.username + '&password=' + that.pwd).then((res) => {
          ...
          storage.set("id", res.customer.id);
          this.$emit('logining');
        }, (rej) => {
          that.pwdisshow = true;
    ...
    timer() {
      if (this.time > 0 && this.timer) {
        ...
        $(this.$refs.senbtn).css("background","#D1D1D1");
        $(this.$refs.senbtn).css("border","#D1D1D1 1px solid");
      } else {
        this.time = 0;
        this.btntxt = "获取验证码";
        this.disabled = false;
        $(this.$refs.senbtn).css("background","#009FE2");
        $(this.$refs.senbtn).css("border","#009FE2 1px solid");
      }
    },
    query() {
